...
#location: drozer/src/drozer/modules/common
    ...
    class ServiceBindingProxy(object):
        ...
        def add_extra(self, extra):
            ...
            if extra[0] == "integer":
                ...
                self.bundle.putBoolean(extra[1], bool(extra[2] == "true")) 
            elif extra[0] == "string":
        ...
        def add_extra_bundle(self, contents, name):
            if self.bundle == None:
                self.bundle = self.context.new("android.os.Bundle")

            dat_bundle = self.context.new("android.os.Bundle")
                
            for extra in contents:

                extra = extra.split(";")
                if len(extra) != 3:
                    raise TypeError
                if extra[0] == "integer":
                    dat_bundle.putInt(extra[1], int(extra[2]))
                elif extra[0] == "short":
                    dat_bundle.putShort(extra[1], int(extra[2]))
                elif extra[0] == "float":
                    dat_bundle.putFloat(extra[1], float(extra[2]))
                elif extra[0] == "double":
                    dat_bundle.putDouble(extra[1], float(extra[2]))
                elif extra[0] == "boolean":
                    print extra[0], extra[1], extra[2]
                    dat_bundle.putBoolean(extra[1], extra[2] == "true")
                elif extra[0] == "string":
                    dat_bundle.putString(extra[1], extra[2])
                elif extra[0] == "byte":
                    dat_bundle.putByte(extra[1], extra[2])
                elif extra[0] == "char":
                    dat_bundle.putChar(extra[1], extra[2])
                else:
                    raise TypeError
            self.bundle.putBundle(name, dat_bundle)
